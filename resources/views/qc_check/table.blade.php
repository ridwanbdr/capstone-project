  ...
  <tbody>
    @forelse($qcChecks as $qc)
      <tr>
        ...
        <td class="border-bottom-0">
          <p class="mb-0 fw-normal">{{ $qc->product_name }}</p>
        </td>
        <td class="border-bottom-0">
        ...
        <td class="border-bottom-0">
          <p class="mb-0 fw-normal">{{ $qc->checker?->nama_lengkap ?? '-' }}</p>
        </td>
        <td class="border-bottom-0">
          @if ($qc->qc_label === 'PASS')
        ...
        <td class="border-bottom-0">
          <a href="{{ route('qc_check.edit', $qc->id) }}" class="btn btn-sm btn-info">
            <i class="ti ti-edit"></i> Edit
          </a>
          <button type="button" class="btn btn-sm btn-danger" onclick="deleteQc({{ $qc->id }})">
